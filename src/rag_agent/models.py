...
class Membership(Base, TimestampMixin):
    ...
    organization_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey('organizations.id', ondelete='CASCADE'))
    workspace_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey('workspaces.id', ondelete='CASCADE'))
...
class Document(Base, TimestampMixin):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    workspace_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey('workspaces.id', ondelete='CASCADE'))
    created_by_user_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey('users.id', ondelete='RESTRICT'))
...
class DocumentChunk(Base, TimestampMixin):
    ...
    __table_args__ = (Index('ix_document_chunks_workspace_document', 'workspace_id', 'document_id'),)
...
class IngestionJob(Base, TimestampMixin):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    workspace_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey('workspaces.id', ondelete='CASCADE'))
    requested_by_user_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey('users.id', ondelete='RESTRICT'))
