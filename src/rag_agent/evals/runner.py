...
def build_eval_service(dataset: EvalDataset, settings: Settings) -> tuple[RAGService, str, str]:
    ...
    user_id = store.get_user_by_email('eval@example.com').id
...
def run_evaluation(dataset_path: Path | str, settings: Settings) -> EvaluationReport:
    ...
    for case in dataset.cases:
        result = service.query(user_id=user_id, workspace_id=workspace_id, query=case.query, request_id=case.case_id)
    ...
    metrics = {
        ...
        'latency_p95_ms': sorted(case.metrics.latency_ms for case in cases)[max(0, int(len(cases) * 0.95) - 1)],
        'token_usage_total': sum(case.metrics.total_tokens for case in cases),
        'estimated_cost_usd_total': sum(case.metrics.estimated_cost_usd for case in cases),
    }
    return EvaluationReport(dataset_name=dataset.name, metrics=metrics, cases=cases)
