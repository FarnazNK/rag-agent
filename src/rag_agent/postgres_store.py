...
from rag_agent.models import Document, DocumentChunk, IngestionJob, Membership, Organization, User, Workspace
...
class PostgresRAGStore:
    ...
    def create_workspace(self, organization_id: str, slug: str, name: str) -> WorkspaceRecord:
        def inner():
            with self._session() as session:
                workspace = Workspace(organization_id=organization_id, slug=slug, name=name, index_version=1)
    ...
    def add_membership(self, user_id: str, organization_id: str, workspace_id: str, role: MembershipRole) -> MembershipRecord:
        def inner():
            with self._session() as session:
                ...
                return MembershipRecord.model_validate({**membership.__dict__, 'role': membership.role})
        return self._run(inner)
    ...
    def get_membership(self, user_id: str, workspace_id: str) -> MembershipRecord | None:
        def inner():
            with self._session() as session:
                ...
                return (
                    MembershipRecord.model_validate({**membership.__dict__, 'role': membership.role})
                    if membership
                    else None
                )
        return self._run(inner)
    ...
    def update_document(self, document_id: str, **updates) -> DocumentRecord:
        def inner():
            with self._session() as session:
                ...
                session.execute(update(Document).where(Document.id == document_id).values(**updates))
                session.commit()
    ...
    def list_documents(self, workspace_id: str) -> list[DocumentRecord]:
        def inner():
            with self._session() as session:
                docs = session.scalars(select(Document).where(Document.workspace_id == workspace_id)).all()
    ...
    def dense_search(self, workspace_id: str, query_embedding: list[float], limit: int) -> tuple[list[RetrievedChunk], float]:
        ...
        def inner():
            ...
            with self._session() as session:
                rows = session.execute(sql, {'workspace_id': workspace_id, 'embedding': vector_literal, 'limit': limit}).mappings().all()
    ...
    def lexical_search(self, workspace_id: str, query: str, limit: int) -> tuple[list[RetrievedChunk], float]:
        ...
        def inner():
            start = time.perf_counter()
            with self._session() as session:
                rows = session.execute(sql, {'workspace_id': workspace_id, 'query': query, 'limit': limit}).mappings().all()
            latency_ms = (time.perf_counter() - start) * 1000
            return [
                RetrievedChunk(
                    chunk_id=row['id'],
        ...
        def inner():
            with self._session() as session:
                if 'status' in updates and hasattr(updates['status'], 'value'):
                    updates['status'] = updates['status'].value
                updates['updated_at'] = datetime.now(UTC)
                session.execute(update(IngestionJob).where(IngestionJob.id == job_id).values(**updates))
                session.commit()
    ...
    def corpus_counts(self) -> tuple[int, int]:
        def inner():
            with self._session() as session:
                docs = int(session.scalar(select(func.count()).select_from(Document).where(Document.status == 'ready')) or 0)
