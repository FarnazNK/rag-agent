...
def create_app(service: RAGService | None = None) -> FastAPI:
    ...
    async def bootstrap(req: BootstrapRequest, svc: RAGService = Depends(get_service)):
    ...
    async def login(req: LoginRequest, svc: RAGService = Depends(get_service)):
    ...
    async def upload_document(
        ...
        try:
            document, job = svc.ingest_document(
                user_id=user.id,
    ...
    async def stream(
        ...
        async def event_generator():
            ...
            done = {"request_id": result.request_id, "cached": result.cached}
