...
class DeterministicEmbeddingProvider:
    ...
    def _embed(self, text: str) -> list[float]:
        ...
        return [((digest[i % len(digest)] / 255.0) * 2) - 1 for i in range(self.dimensions)]
...
class OpenAIEmbeddingProvider:
    ...
    @retry(
        ...
        retry=retry_if_exception_type((TimeoutError, ConnectionError, EmbeddingProviderError, ProviderRateLimitError)),
        reraise=True,
    )
    def embed_documents(self, texts: list[str]) -> list[list[float]]:
        try:
            ...
            if 'rate limit' in message or '429' in message:
                raise ProviderRateLimitError('Embedding provider rate limited the request.') from exc
...
class EmbeddingService:
    def __init__(self, provider: EmbeddingProvider, cache: TTLCache[str, list[float]] | None = None) -> None:
...
def build_embedding_service(settings: Settings | None = None) -> EmbeddingService:
    ...
    if settings.embedding_provider == 'deterministic':
        return EmbeddingService(DeterministicEmbeddingProvider(settings.embedding_dimensions), cache=cache)
