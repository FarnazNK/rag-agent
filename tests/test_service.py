...
def test_cache_hit_and_invalidation(seeded_app):
    ...
    document = seeded_app.service.list_documents(
        user_id=seeded_app.alice_id, workspace_id=seeded_app.alice_workspace_id
    )[0]
...
def test_embedding_failure_marks_ingestion_failed():
    ...
    service = RAGService(
        store, settings=settings, embedding_service=EmbeddingService(FailingEmbeddingProvider())
    )
...
def test_token_limit_exceeded():
    ...
    seeded.service.settings = seeded.service.settings.model_copy(
        update={"llm_max_prompt_tokens": 4}
    )
